...
# Read the input file
$lines = Get-Content -Path .\day_3\input.txt
...
foreach ($line in $lines) {
    ...
    $matches = [regex]::Matches($line, 'mul\((\d+),(\d+)\)')
...
foreach ($line in $lines) {
    # Find all the matches of the pattern
    $matches = [regex]::Matches($line, "mul\((\d+),(\d+)\)|do\(\)|don\'t\(\)")
    ...
    foreach ($match in $matches) {
        if ($match.Value -eq "do()") {
            ...
            # Check if the instruction is enabled
            if ($enabled) {
                # Get the two numbers
                $num1 = [int]$match.Groups[1].Value
                $num2 = [int]$match.Groups[2].Value
