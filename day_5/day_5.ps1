...
specific order. The notation X|Y means that if both page number X and page number Y are to be 
...
printed at some point before page number 53. (47 doesn't necessarily need to be immediately before 
...
In the above example, the first update (75,47,61,53,29) is in the right order:
    ...
    75 is correctly first because there are rules that put each other page after it: 75|47, 75|61, 
    75|53, and 75|29.
    
    47 is correctly second because 75 must be before it (75|47) and every other page must be after 
    it according to 47|61, 47|53, and 47|29.
    
    61 is correctly in the middle because 75 and 47 are before it (75|61 and 47|61) and 53 and 29 
    are after it (61|53 and 61|29).
...
$rules = Get-Content -Path .\day_5\input.txt | Select-String -Pattern '^\d+\|\d+$' | ForEach-Object {
...
$updates = Get-Content -Path .\day_5\input.txt | Select-String -Pattern '^\d+(,\d+)*$' | ForEach-Object {
    $_.Matches.Value
}
...
foreach($update in $updates) {
    ...
    foreach($rule in $rules) {
        ...
        $beforeIndex = [array]::IndexOf($update, $rule.Before)
        $afterIndex = [array]::IndexOf($update, $rule.After)

        # If before page is after after page, update is not in order
        if($beforeIndex -ne -1 -and $afterIndex -ne -1 -and $beforeIndex -gt $afterIndex) {
            $inOrder = $false
            break
        }
    }

    # If update is in order, find middle page and add to sum
    if($inOrder) {
...
While the Elves get to work printing the correctly-ordered updates, you have a little time to fix 
the rest of them.

For each of the incorrectly-ordered updates, use the page ordering rules to put the page numbers in 
the right order. For the above example, here are the three incorrectly-ordered updates and their 
correct orderings:
...
foreach($update in $updates) {
    ...
    # If update is not in order, order it correctly
    if(-not $inOrder) {
        $orderedUpdate = New-Object System.Collections.ArrayList
        $orderedUpdate.Add($update[0]) | Out-Null

        # Iterate over pages in update
        for($i = 1; $i -lt $update.Length; $i++) {
            $beforePos = $i
            $afterPos = 0
            $page = $update[$i]
            ...
            if($beforePos -gt $afterPos) {
                $orderedUpdate.Insert($beforePos, $page) | Out-Null 
            } else {
                $orderedUpdate.Insert($afterPos, $page) | Out-Null 
            }
        }

        # Find middle page and add to sum
        $middleIndex = [math]::Floor($orderedUpdate.Count / 2)
        $middlePage = $orderedUpdate[$middleIndex]
        $sum += $middlePage
    }
}
